...
const batchRequestSchema = z.object({
  emails: z.array(batchEmailSchema).min(1).max(1000),
});
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[/api/emails/batch] Queuing batch of ${emails.length} emails for domain=${domain.domain}`
    );
    ...
    return cors(
      NextResponse.json(
        { batchId, total: emails.length, status: "pending" },
        { status: 202 }
      )
    );
  } catch (error: unknown) {
